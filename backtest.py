...
class Backtest:
    def __init__(
        ...
        self.instruction_pending:  Optional[deque[Instruction]] = deque()
        self.instruction_completed:  Optional[deque[Instruction]] = deque()
        self.instruction_cancelled: Optional[deque[Instruction]] = deque()
        self.priority_qty_per_Buyprice: Optional[dict[float, float]] = {}
        self.priority_qty_per_Sellprice: Optional[dict[float, float]] = {}
        ...
        self.dataframe: Optional[list] = []  # TODO this is for test
        self.dataframe_trade: Optional[list] = []
    ...
    async def read_trade_data(self):
         
        await self.dataLoader.tick_trade_data_feed_nonAsync()
    ...
    #     for trade_data in self.dataLoader.tick_trade_data_feed():
            
    #         if trade_data == None: continue
            
    #         if trade_data[start_index] > str(self.market_data.order_book.ts): break
    #         if trade_data[start_index] == str(self.market_data.order_book.ts):
    #             trade_list.append((trade_data[price_index], trade_data[qty_index]))
    #             self.dataframe_trade.append(trade_data)
    #     # trade_list_sorted = sorted(trade_list, key=lambda x:x[0])  # price ascending
    #     self.trade_list = trade_list  # TODO 如何结合depth data  以及是否要舍弃排序
    ...
    async def before_tick(self):
        ...
        while(self.trade_index < len(self.dataLoader.trade_dataframe)):
            trade_data = self.dataLoader.trade_dataframe[self.trade_index]
        ...
        self.trade_list = trade_list  # TODO 如何结合depth data  以及是否要舍弃排序
        self.trade_bid_dict = dict(zip(self.market_data.order_book.get_price_list('bid'), [[(x, 'depth')] for x in self.market_data.order_book.get_vol_list('bid')]))
        self.trade_ask_dict = dict(zip(self.market_data.order_book.get_price_list('ask'), [[(x, 'depth')] for x in self.market_data.order_book.get_vol_list('ask')]))  # TODO 没有inst存在的价位用depth来替
    ...
    async def post_tick(self):
        ...
        await self.account.update_metric()
    ...
    async def start_test(self):
        ...
        async for depth_data in self.dataLoader.tick_depth_data_feed():
            if depth_data == None:   # TODO not in the target backtest time interval
                ...
                self.dataframe.append(depth_data)
              
                if index == 50:
                    pd.DataFrame(self.dataframe).to_csv('test_depth.csv')
                    pd.DataFrame(self.dataframe_trade).to_csv('test_trade.csv')
        
                if len(depth_data) < len(self.dataLoader.depth_cols) : 
                                       # TODO 暂时先跳过bid ask档位缺失的depth data
                    continue
    ...
    async def start(self):
        ...
        print(self.account.get_current_net_worth())
